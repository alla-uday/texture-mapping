...
Declare_Any_Class( "Example_Animation",  // An example of a displayable object that our class Canvas_Manager can manage.  This one draws the scene's 3D shapes.
  { 'construct': function( context )
      { this.shared_scratchpad    = context.shared_scratchpad;
        ...
        shapes_in_use.cube = new Cube(0);
        shapes_in_use.cube1 = new Cube(0.5);
    ...
    'init_keys': function( controls )   // init_keys():  Define any extra keyboard shortcuts here
      {
        ...
        controls.add( "r", this, function() { this.shared_scratchpad.animate                      ^= 1; } );
      },
    'update_strings': function( user_interface_string_manager )       // Strings that this displayable object (Animation) contributes to the UI:
      {
    ...
    'display': function(time)
      {
        ...
        var purplePlastic = new Material( Color( .9,.5,.9,1 ), .4, .4, .8, 40 ), // Omit the final (string) parameter if you want no texture
              ...
              backgroundNearest = new Material(Color(0,0,0,1), 1,0,0,40, "boulders.png"),
              backgroundMipMap = new Material(Color(0,0,0,1), 1,0,0,40, "boulders2.png"),
        ...
        **********************************/                                     // From here on down it's just some example shapes drawn for you -- replace them with your own!
        ...
        //model_transform = mult(model_transform, rotation(degreeOfRotation, [0,1,0])); 
        //degreesOfRotation[0] = degreesOfRotation[0] + degRatesOfEachCube[0];
        shapes_in_use.cube       .draw( graphics_state, mult( model_transform, rotation( .02*graphics_state.animation_time, 0, 1, 0 ) ), backgroundNearest );

        model_transform = mult( model_transform, translation( 4, 0, 0 ) );
        shapes_in_use.cube1          .draw( graphics_state, mult( model_transform, rotation( .03*graphics_state.animation_time, 1, 0, 0 ) ), backgroundMipMap );
