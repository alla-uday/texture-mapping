...
Declare_Any_Class( "Cube",    // A square, demonstrating shared vertices.  On any planar surface, the interior edges don't make any important seams.
  { 'populate': function(zoom)      // In these cases there's no reason not to re-use values of the common vertices between triangles.  This makes all the
      {                         // vertex arrays (position, normals, etc) smaller and more cache friendly.
         var side = 1;
         var verticesOfCube = [
         ...
         this.normals       .push( vec3(-1,0,0), vec3(-1,0,0), vec3(-1,0,0), vec3(-1,0,0) );     // ...
          ...
          this.texture_coords.push(vec2(0 - zoom,1+ zoom), vec2(0 - zoom,0 - zoom), vec2(1 + zoom ,0 - zoom), vec2(1 + zoom ,1 + zoom));
          this.texture_coords.push(vec2(0 - zoom,1+ zoom), vec2(0 - zoom,0 - zoom), vec2(1 + zoom ,0 - zoom), vec2(1 + zoom ,1 + zoom));
          this.texture_coords.push(vec2(0 - zoom,1+ zoom), vec2(0 - zoom,0 - zoom), vec2(1 + zoom ,0 - zoom), vec2(1 + zoom ,1 + zoom));
          this.texture_coords.push(vec2(0 - zoom,1+ zoom), vec2(0 - zoom,0 - zoom), vec2(1 + zoom ,0 - zoom), vec2(1 + zoom ,1 + zoom));
          this.texture_coords.push(vec2(0 - zoom,1+ zoom), vec2(0 - zoom,0 - zoom), vec2(1 + zoom ,0 - zoom), vec2(1 + zoom ,1 + zoom));
